...
| 3 | Printer sharing must cross departments; file sharing must not | 🟢 Brief §8 | Need one resource reachable by both departments and one resource isolated per department | 🟠 Dedicated Printer VLAN reachable from both Admin and Technical; inter-department file-sharing (FTP/SMB) between Admin and Technical denied by extended ACL 102, while shared printer access and general management/ICMP traffic remain permitted |
...
| 8 | CR14: after-hours contractors require limited wireless access, addressed via the DHCP design, preserving existing segmentation | 🟢 Brief §10 | Contractor segment must receive a DHCP-issued address and must not reach internal department resources | Contractor VLAN scoped by DHCP like any other VLAN; ACL 100 denies contractor traffic to the Admin server (172.30.98.2) while general internet access is permitted (🟠 my own interpretation of "limited access," documented in the logical topology) |
| 9 | Deliberate configuration fault, documented, then resolved | 🟢 Brief §9 | Project must include one troubleshooting cycle with evidence | Executed & resolved: removed `ip helper-address` from `Kopano-Edge-R1`'s Gi0/0.20 sub-interface, producing a clean, isolated failure (VLAN 20 clients defaulted to 169.254.x.x while other VLANs kept working); resolved by re-adding the helper-address (README §4) |
| 10 | At least one negative test with explanation | 🟢 Brief §9, §11 | Need at least one test case that is expected to fail | Executed: denied file-share access attempt between Admin and Technical documented as the negative test — TEST-04 (FTP blocked via ACL 102, 24 match hits) in README §5 |
...
The implemented design, device configuration, and verification results are documented in the repository `README.md`: the full test matrix (TEST-01 through TEST-06, including the ACL 102 negative-test match counts) is in README §5, with supporting screenshots under `assets/` (see `assets/README.md`).
